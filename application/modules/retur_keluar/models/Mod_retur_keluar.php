...
class Mod_retur_keluar extends CI_Model
{
	...
	private function _get_datatables_query($term='') //term is value of $_REQUEST['search']
	{
		...
		$this->db->join(
			'tbl_retur_keluar_detail',
			'tbl_retur_keluar.id_retur_keluar = tbl_retur_keluar_detail.id_retur_keluar','left');
		$this->db->join(
			'tbl_barang',
			'tbl_retur_keluar_detail.id_barang = tbl_barang.id_barang','left');
		$this->db->join(
			'tbl_user',
			'tbl_retur_keluar.id_user = tbl_user.id_user','left');
		$this->db->join(
			'tbl_supplier',
			'tbl_retur_keluar.id_supplier = tbl_supplier.id_supplier','left');

		$this->db->like('tbl_retur_keluar.id_retur_keluar',$term);
		$this->db->or_like('tbl_barang.nama_barang',$term);
		$this->db->or_like('tbl_user.username',$term);
		$this->db->or_like('tbl_retur_keluar.tgl_retur_keluar',$term);
		$this->db->or_like('tbl_supplier.nama_supplier',$term);
		$this->db->group_by('tbl_retur_keluar.id_retur_keluar');
	...
	public function get_list_penerimaan($id_trans_masuk)
	{
		$this->db->select('
			tbl_trans_masuk_detail.id_trans_masuk_detail,
			tbl_trans_masuk_detail.id_trans_masuk,
			tbl_trans_masuk_detail.id_barang, 
			tbl_barang.nama_barang,
		...
		$this->db->join(
			'tbl_barang',
			'tbl_trans_masuk_detail.id_barang = tbl_barang.id_barang',
			'left');
		$this->db->join(
			'tbl_satuan',
			'tbl_trans_masuk_detail.id_satuan = tbl_satuan.id_satuan',
			'left');
		$this->db->where('tbl_trans_masuk_detail.id_trans_masuk', $id_trans_masuk);
        $query = $this->db->get();
	...
	public function get_list_supplier($id_trans_masuk)
	{
		$this->db->select('
			tbl_trans_masuk.id_trans_masuk,
			tbl_trans_masuk.id_supplier,
			tbl_supplier.nama_supplier,
		');
		$this->db->from('tbl_trans_masuk');
		$this->db->join(
			'tbl_supplier',
			'tbl_trans_masuk.id_supplier = tbl_supplier.id_supplier',
			'left');
	...
	public function get_detail_retur_keluar($id_retur_keluar)
	{
		...
		$this->db->join('tbl_retur_keluar', 'tbl_retur_keluar.id_retur_keluar = tbl_retur_keluar_detail.id_retur_keluar','left');
		$this->db->join('tbl_barang', 'tbl_barang.id_barang = tbl_retur_keluar_detail.id_barang','left');
		$this->db->join('tbl_satuan', 'tbl_satuan.id_satuan = tbl_retur_keluar_detail.id_satuan','left');
	...
	public function get_detail_retur_keluar_header($id_retur_keluar)
	{
		$this->db->select('tbl_retur_keluar.id_retur_keluar,
							tbl_retur_keluar.tgl_retur_keluar,
							tbl_retur_keluar_detail.id_trans_masuk,
							tbl_user.id_user,
							tbl_user.username,
		...
		$this->db->join('tbl_user_detail', 'tbl_retur_keluar.id_user = tbl_user_detail.id_user','left');
	...
	public function lookup_id_penerimaan($keyword = "")
	{
		$this->db->select('id_trans_masuk');
		$this->db->from('tbl_trans_masuk_detail');
		$this->db->like('id_trans_masuk',$keyword);
		//$this->db->limit(5);
		$this->db->order_by('id_trans_masuk', 'DESC');
		$this->db->group_by('id_trans_masuk');
		$query = $this->db->get();
		return $query->result();
	}
	...
	public function save($data_retur_keluar, $data_retur_keluar_detail)
	{

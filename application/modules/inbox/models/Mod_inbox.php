...
class Mod_inbox extends CI_Model
{
	...
	private function _get_datatables_query($term="", $id_user) //term is value of $_REQUEST['search']
	{
		...
		$this->db->order_by('tbl_pesan.dt_post', 'desc');
	...
	{
		$this->db->select('tbl_pesan.id_pesan, tbl_pesan.id_user, tbl_pesan.id_user_target, tbl_user_detail.nama_lengkap_user, tbl_pesan.subject_pesan, tbl_pesan.isi_pesan, tbl_pesan.dt_post, tbl_pesan.dt_read');
		$this->db->from('tbl_pesan');
		$this->db->join('tbl_user_detail', 'tbl_pesan.id_user = tbl_user_detail.id_user', 'left');
	...
	public function update_data_inbox($where, $data)
	{

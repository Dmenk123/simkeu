...
class Forecasting extends CI_Controller {
	...
	public function forecasting_step_2()
	{
		...
		$angka = 0;
		$alpha_result = array();
		foreach($hasil_alpha as $x => $x_value) {
		    // echo "Key=" . $x . ", Value=" . $x_value;
		    $alpha_result[$angka] = $x;
		 	$angka++;
		}
		

		/*$nilai_min_alpha = ""; //solusi variabel alpha tidak terbaca ketika data tidak ada transaksi
		//loop jika kondisi true, simpan variabel yang berisi nilai key
		while ($cari_min_alpha = current($hasil_alpha)) {
		    if ($cari_min_alpha == $hasil_alpha_sort) {
		...
		if ($this->session->userdata('id_level_user') == 1 || $this->session->userdata('id_level_user') == 2 || $this->session->userdata('id_level_user') == 3 ) {
			$data = array(
				...
				'qty_notif' => $jumlah_notif,
				'isi_notif' => $notif,
				'alpha_1' => $alpha_result['0'],
				'alpha_2' => $alpha_result['1'],
				'alpha_3' => $alpha_result['2'],
			);
		}
		$this->load->view('view_home',$data);
	}

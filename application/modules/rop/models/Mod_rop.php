...
class Mod_rop extends CI_Model
{
	...
	{
		$this->db->select('tbl_barang.id_barang, tbl_barang.nama_barang, tbl_barang.id_satuan, tbl_satuan.nama_satuan');
	...
	public function get_pengeluaran_barang($id_barang, $bulan_index, $period_tampil)
	{
		$query =  $this->db->query(
		"SELECT 
	...
	/*public function get_pengeluaran_barang($id_barang, $bulan_index, $period_tampil)
	{
		...
		FROM (
  			...
  			WHERE a.Date <= '".$bulan_index."' and a.Date >= Date_add('".$bulan_index."',interval - 3 month)
  			GROUP BY month
			)t1
		...
		LEFT JOIN (
			SELECT
		    	...
		    	DATE_FORMAT(tgl_trans_keluar_detail, '%Y') as tahun
  			FROM tbl_trans_keluar_detail
  			WHERE tgl_trans_keluar_detail <= '".$bulan_index."' and tgl_trans_keluar_detail >= Date_add('".$bulan_index."',interval - 3 month) AND id_barang = '".$id_barang."'
  			GROUP BY md
		)t2
		ON t2.md = t1.md 
		GROUP BY t1.month
